class UsersController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @user.save and @user.create_viewparam(notification_view: '0') and create_box(params[:user][:email]) and @user.groups.create(name: 'My first group') and @user.groups.last.groupfolders.create(name: Group.last.name, parent_id: 100) and Group.last.create_calendar(name: 'group calendar')
